...
export function MiniModel() {
  ...
  return (
    <Canvas shadows camera={{ position: [5, 0, 15], fov: 30 }}>
      ...
      <Mini scale={2} position={[-0.14, -0.22, 1]} rotation={[0, -Math.PI / 4, 0]} />
      <AccumulativeShadows position={[0, -1.16, 0]} frames={50} alphaTest={0.9} scale={10}>
      ...
      <color attach="background" args={[theme === "dark" ? "#0f1217" : "white"]} />
      <CameraRig />
    </Canvas>
  );
}
...
function CameraRig({ v = new THREE.Vector3() }) {
  return useFrame((state) => {
    ...
    state.camera.position.lerp(v.set(Math.sin(t / 2) * 1.2, 0, 12 + Math.cos(t / 5) / 2), 0.05);
